#!/usr/bin/env python3
"""
Dr. Strunz Knowledge Base MCP Server - Version 3.0
Complete rewrite using official FastMCP patterns with proper session management
Fixes all MCP protocol communication issues
"""
...
mcp_server = FastMCP(
    name="Dr. Strunz Knowledge Server v3.0",
...
# Create lifespan manager for proper resource management
@contextlib.asynccontextmanager
async def lifespan(app: Starlette):
    """Manage application lifecycle"""
    # Startup
    logger.info("Starting Dr. Strunz Knowledge MCP Server v3.0")
    logger.info("Using official MCP Python SDK (FastMCP)")
    await initialize_knowledge_searcher()
    
    # Yield without session manager - FastMCP handles its own lifecycle
    yield
...
app = Starlette(
    routes=[
        ...
        # Mount MCP server with proper SSE support
        Mount("/sse", app=mcp_server.sse_app("/sse")),
        Mount("/mcp", app=mcp_server.sse_app("/mcp")),
    ],
    lifespan=lifespan
)
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting Dr. Strunz Knowledge MCP Server v3.0 on port {port}")

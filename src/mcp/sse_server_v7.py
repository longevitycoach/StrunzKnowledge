#!/usr/bin/env python3
"""
Dr. Strunz Knowledge Base MCP Server - Version 7.0
...
mcp_server = FastMCP(
    name="Dr. Strunz Knowledge Server v0.7.9",
...
def search_knowledge_advanced(
    ...
    try:
        ...
        elif not isinstance(content_types, list):
            return "Error: content_types must be a list of strings"
        
        # Validate each content type
        valid_types = {"books", "news", "forum"}
        content_types = [ct.lower() for ct in content_types if ct.lower() in valid_types]
        
        if not content_types:
            content_types = ["books", "news", "forum"]
        
        logger.info(f"Advanced search for: '{query}' in {content_types} with limit={limit}")
        
        # Perform search with filters - using correct parameter name
        results = knowledge_searcher.search(query=query, k=limit, sources=content_types)
...
def get_book_content(
    ...
    try:
        ...
        if not book_title or not isinstance(book_title, str):
            return "Error: Book title must be a non-empty string"
        ...
        logger.info(f"Getting book content for: '{book_title}' pages {page_range}")
        
        # Search for book content specifically
        query = f"{book_title} {page_range}"
        results = knowledge_searcher.search(query=query, k=20, sources=["books"])
        ...
        for result in results:
            ...
            if page in seen_pages and page != 'Unknown':
                continue
            seen_pages.add(page)
...
{chr(10).join(formatted_results)}"""
        ...
        logger.error(f"Book content error: {e}", exc_info=True)
...
def search_news(
    ...
    try:
        ...
        logger.info(f"Searching news for: '{query}' with limit={limit}")
        
        # Search news specifically
        results = knowledge_searcher.search(query=query, k=limit, sources=["news"])
...
- books: Dr. Strunz's published books
- news: Articles from strunz.com
...
async def startup_event():
    ...
    logger.info("Starting Dr. Strunz Knowledge MCP Server v7.0")
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting Dr. Strunz Knowledge MCP Server v7.0 on port {port}")
